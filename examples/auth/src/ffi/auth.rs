...
impl GopherAuthClient {
    ...
    pub fn new(jwks_uri: &str, issuer: &str) -> Result<Self, AppError> {
        ...
        unsafe {
            let init: Symbol<GopherAuthInitFn> = library
                ...
                .map_err(|e| AppError::Ffi(format!("Failed to load gopher_auth_init: {}", e)))?;

            let result = init();
            if result != 0 {
                return Err(AppError::Ffi(format!(
        ...
        let jwks_uri_c = CString::new(jwks_uri)
            .map_err(|e| AppError::Ffi(format!("Invalid jwks_uri: {}", e)))?;
        let issuer_c = CString::new(issuer)
            .map_err(|e| AppError::Ffi(format!("Invalid issuer: {}", e)))?;

        let handle = unsafe {
            let create: Symbol<GopherAuthClientCreateFn> = library
                .get(b"gopher_auth_client_create\0")
                .map_err(|e| {
                    AppError::Ffi(format!("Failed to load gopher_auth_client_create: {}", e))
                })?;
    ...
    fn load_library() -> Result<Library, AppError> {
    ...
    pub fn extract_payload(&self, token: &str) -> Result<TokenPayload, AppError> {
        let token_c = CString::new(token)
            .map_err(|e| AppError::Ffi(format!("Invalid token string: {}", e)))?;

        unsafe {
            let extract: Symbol<GopherAuthExtractPayloadFn> = self
                .library
                .get(b"gopher_auth_extract_payload\0")
                .map_err(|e| AppError::Ffi(format!("Failed to load extract function: {}", e)))?;
    ...
    unsafe fn get_payload_string(&self, payload: *mut c_void, fn_name: &[u8]) -> Result<String, AppError> {
        let get_fn: Symbol<GopherAuthPayloadGetSubjectFn> = self
            .library
            .get(fn_name)
            .map_err(|e| AppError::Ffi(format!("Failed to load getter function: {}", e)))?;
    ...
    unsafe fn get_payload_expiration(&self, payload: *mut c_void) -> Result<u64, AppError> {
        let get_fn: Symbol<GopherAuthPayloadGetExpirationFn> = self
            .library
            .get(b"gopher_auth_payload_get_expiration\0")
            .map_err(|e| AppError::Ffi(format!("Failed to load expiration getter: {}", e)))?;
    ...
    pub fn set_option(&self, key: &str, value: &str) -> Result<(), AppError> {
        let key_c = CString::new(key)
            .map_err(|e| AppError::Ffi(format!("Invalid option key: {}", e)))?;
        let value_c = CString::new(value)
            .map_err(|e| AppError::Ffi(format!("Invalid option value: {}", e)))?;

        unsafe {
            let set_option: Symbol<GopherAuthSetOptionFn> = self
                .library
                .get(b"gopher_auth_set_option\0")
                .map_err(|e| AppError::Ffi(format!("Failed to load set_option: {}", e)))?;
